...
- `cupboard cache` manages named caches and the retention each one gives its
  roots. `cupboard root` manages what a cache keeps.
...
the cache name as the next positional argument. A first positional that names an
existing file or directory is a local path. Otherwise it is read as a cache name
when a cache of that name exists; pass `./result` or the `/cache/<name>` URL to
