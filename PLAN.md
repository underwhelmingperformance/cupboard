...
- The deploy applies the transitions in order within one run
  ...
  Workers.
- The Workers check whether a transition is complete (`TransitionGate`,
  `hasReached('cache-identity', 'complete')`). The required local step is the
...
- `pnpm check:migrations` fails when the files, the journal and the transitions
  ...
  the same transition. It also replays the deploy's order for each N, starting
  from a deployment whose first N transitions are complete and whose later ones
...
- [ ] Before a transition after `deployment-transitions` that has contract
      migrations is added, make `check:migrations` replay every history that a
      deployment can have. The replay does not yet cover an independent
      transition with contract migrations that expanded ahead of an earlier
      transition's contract migrations and is still expanded when a later
      release adds another transition.
