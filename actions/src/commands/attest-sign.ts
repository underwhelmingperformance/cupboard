...
export function registerAttestSignCommand(
	...
	program
		...
		.option(
			...
			"Take the signing defaults from the destination cache's access. Accepts public or private. If omitted, use the private defaults, which do not publish a bundle."
		)
		.option(
...
/**
 * Resolves the signing policy for this run. The destination's access supplies
 ...
 *
 * When no access is given, the resolver uses the private defaults, which do
 * not publish a bundle. With the public defaults, an omitted value could
 * publish a destination cache's NAR hashes, and no later step can retract
 * them.
 */
