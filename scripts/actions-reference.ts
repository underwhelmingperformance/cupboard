...
// Render the reference from the action and workflow definitions, so the
// documented inputs always match what GitHub accepts.
...
interface PublicWorkflow {
	...
	// Workflow files have a name but no description, so describe them here.
...
// Collapse a YAML description to one line. Escape `<` so that a placeholder
// such as `<number>` isn't read as an HTML tag.
...
// The caller has to grant every permission that any job in the workflow asks
// for, so report the highest level asked for in each scope.
...
export function renderActionsReference(sources: ReferenceSources): string {
	...
	return `${[
		...
		'## Internal actions',
		`${internal} are building blocks of the reusable workflows. Their inputs can change in any release, so don't call them directly. Use the reusable workflows instead.`
	].join('\n\n')}\n`;
}
