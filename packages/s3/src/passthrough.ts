...
/**
 * A reference {@link ObjectStore} that relays every operation straight to a
 * backing S3 service (e.g. R2's S3 API), signing with SigV4. Pairing it with
 * `createS3App` yields a transparent proxy: the conformance oracle for the
 * generic protocol layer. Not for production use.
 */
export function createPassthroughStore(
	...
	return {
		...
		async uploadPart(
			...
			const response = await send('PUT', url, { body });
			assertOk(response);
