...
/**
 * Serialises a document with one root element and the standard XML declaration.
 * `fast-xml-parser` renders the elements and escapes entities.
 */
...
const completeUploadSchema = z.object({
	CompleteMultipartUpload: z.object({
		Part: z.array(completedPartSchema).min(1).max(10_000)
	})
});
...
/**
 * Parses a `CompleteMultipartUpload` request body into its part list.
 */
