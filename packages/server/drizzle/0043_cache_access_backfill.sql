...
--
-- Each statement below inserts 0 into this table when a precondition does not
-- hold, and the CHECK then fails the migration before any data is changed. The
-- failure reports only "CHECK constraint failed", so the comment on each
-- statement says which precondition fired.
CREATE TABLE `_cache_access_migration_assertion` (
	`valid` integer NOT NULL CHECK (`valid` = 1)
);--> statement-breakpoint
-- Every legacy cache name the store refers to: the `cache` rows and every
-- data table keyed by cache, so a cache deleted before this migration ran is
-- still counted. The precondition below and the deleted-identity insert both
-- read this union.
CREATE TABLE `_legacy_cache_names` (
	`legacy_cache` text PRIMARY KEY NOT NULL
);--> statement-breakpoint
INSERT OR IGNORE INTO `_legacy_cache_names` SELECT `name` FROM `cache`;--> statement-breakpoint
...
-- One cache name must not appear both public and private, whether the
-- other side is a live `cache` row or only data left by a deleted cache.
INSERT INTO `_cache_access_migration_assertion` (`valid`)
SELECT 0
WHERE EXISTS (
	SELECT 1
	FROM `_legacy_cache_names`
	WHERE `legacy_cache` <> ''
	GROUP BY CASE WHEN `legacy_cache` LIKE 'private/%' THEN substr(`legacy_cache`, 9) ELSE `legacy_cache` END
	HAVING min(`legacy_cache` LIKE 'private/%') <> max(`legacy_cache` LIKE 'private/%')
);--> statement-breakpoint
-- One reuse view name must not appear both public and private.
INSERT INTO `_cache_access_migration_assertion` (`valid`)
SELECT 0
WHERE EXISTS (
	SELECT 1
...
-- A cache name that appears only in data, with no row in `cache`, was deleted
-- before this migration ran. Register it as an already-deleted identity so the
-- rows referring to it resolve, with the epoch standing in for the timestamps
-- the legacy table never recorded.
