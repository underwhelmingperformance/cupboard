...
CREATE TRIGGER `cache_access_mirror_blob_generation_insert`
AFTER INSERT ON `blob_ref`
WHEN NEW.`cache_generation` IS NULL
BEGIN
 UPDATE `blob_ref` SET `cache_generation` = 1 WHERE rowid = NEW.rowid;
END;
