...
-- One cache name must not appear both public and private within a tenant.
...
-- Every named cache must have a name this release can parse.
INSERT INTO `_cache_access_migration_assertion` (`valid`)
SELECT 0
WHERE EXISTS (
	SELECT 1
...
-- Every read credential must be keyed by a private/ name.
INSERT INTO `_cache_access_migration_assertion` (`valid`)
SELECT 0
WHERE EXISTS (
	SELECT 1
