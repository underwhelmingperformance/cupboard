...
// A cache belongs to generation 1 while it has no lifecycle row. Deleting a
// cache advances it to the next generation, so all of its edges stop matching,
// including edges written before `blob_ref.cache_generation` existed.
