...
describe('cache availability query', () => {
	...
	// A cache page costs one narinfo head per hash, and the chunk the Worker
	// sends to the object holds a whole page, so the object answers it in one
	// request.
	it('answers a full page through one object request', async () => {
		const init = await bootstrap();
		const metadata = uploadMetadata({ fileSize: narBytes.byteLength });
