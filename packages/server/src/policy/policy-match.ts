...
function isMatch(policy: RetentionPolicy, root: RootTarget): boolean {
	return policy.scope === 'cache'
		? policy.cache !== undefined && isSameCacheScope(policy.cache, root.cache)
		: policy.pattern !== undefined && root.name.startsWith(policy.pattern);
}

function specificity(policy: RetentionPolicy): number {
	return policy.scope === 'root-name-prefix'
		? (policy.pattern?.length ?? -1)
		: -1;
}
