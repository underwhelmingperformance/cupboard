...
export class CacheAccessMigrationPendingError extends ServerHttpError {
...
export class CacheRetentionRuleLimitExceededError extends ServerHttpError {
	...
	constructor(readonly limit: number) {
		super(
			`A cache supports at most ${String(limit)} root retention overrides. Remove an override before adding another.`
		);
		this.name = 'CacheRetentionRuleLimitExceededError';
	}
}
...
// This build does not define the stored transition id or state, which happens
// after a rollback past a release that added one. To repair it, deploy a build
// that defines it.
export class StoredDeploymentTransitionInvalidError extends ServerHttpError {
	readonly status = StatusCodes.INTERNAL_SERVER_ERROR;
	...
	constructor(
		...
		super(
			`The deployment records transition '${transition}' in state '${state}', which this server version does not know`
		);
		this.name = 'StoredDeploymentTransitionInvalidError';
	}
}
