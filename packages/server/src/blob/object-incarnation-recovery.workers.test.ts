...
const subrequestAllowance = subrequestsPerInvocation(env);
...
describe('abandoned object version recovery', () => {
	...
	it.each([1000])(
		'continues a recovery backlog within a %i-subrequest allowance',
		async (allowance) => {
			const reaperEnv = {
				...env,
				CUPBOARD_SUBREQUESTS_PER_INVOCATION: String(allowance)
			};
			const database = drizzleD1(env.CUPBOARD_DB, { schema: d1Schema });
	...
	it.each([1000])(
		'continues deletion markers within a %i-subrequest allowance',
		async (allowance) => {
			const reaperEnv = {
				...env,
				CUPBOARD_SUBREQUESTS_PER_INVOCATION: String(allowance)
