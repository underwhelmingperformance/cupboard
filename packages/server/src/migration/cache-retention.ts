...
import { CacheRetentionMigrationPendingError } from '../errors.ts';

export const retentionMigrationBatchSize = 50;
export const maximumLegacyRetentionRules = maxRootRetentionOverrides;
...
function hasLegacyRetentionSettings(database: SchemaDatabase): boolean {
	return (
		database
			.select({ id: schema.legacyRetentionPolicies.id })
			.from(schema.legacyRetentionPolicies)
			.limit(1)
			.get() !== undefined ||
		database
			.select({ id: schema.legacyRetentionGracePolicies.id })
...
export function assertRetentionMigrationSettled(
	...
	if (
		...
		(state === undefined && !hasLegacyRetentionSettings(database))
	) {
		return;
	}
	throw new CacheRetentionMigrationPendingError();
}
...
function completeMigration(
	...
	return { status: 'complete', discardedRuleCount: state.discardedRuleCount };
}
...
function graceForCache(
	cache: Pick<
		typeof schema.cacheIdentities.$inferSelect,
		'kind' | 'name' | 'access'
	>,
	policies: readonly (typeof schema.legacyRetentionGracePolicies.$inferSelect)[]
): (typeof schema.cacheIdentities.$inferInsert)['graceSeconds'] | SQL<null> {
...
function advanceCaches(
	...
	// The batch's caches travel as one JSON parameter, so the query binds the
	// same parameters however wide the batch is.
	const defaults = jsonValueLists(cacheIds).flatMap((ids) =>
		database
			.select({
			...
			.where(
				and(
					...
					inArray(schema.legacyRetentionPolicies.cacheId, ids)
				)
			)
			.all()
	);
	...
	return completeMigration(database);
}

/**
 * Advances one bounded batch of the legacy retention migration.
 *
 * Root-prefix policies are imported into one rule set shared by all caches,
 * so a tenant with many caches stores each rule once. Cache defaults and
 * grace are then copied in cache-id order, matching the stored cursor.
 */
export async function advanceCacheRetentionMigration(
	...
	if (state === undefined) {
		...
		if (!hasLegacyRetentionSettings(database)) {
			return completeMigration(database);
		}
	}
