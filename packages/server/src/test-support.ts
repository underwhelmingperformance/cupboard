...
/**
 * Thrown when a maintenance pass is parked behind a retry deadline that the
 * test clock never reaches.
 *
 * A pass that reports a stall is delayed until `now + noProgressRetryMs`. The
 * shared setup pins `Date` so timestamps are deterministic, so that deadline
 * never arrives and the pass does not run again for the rest of the test.
 * Anything that waits on the work the pass left behind then waits until the
 * test times out, and the timeout does not identify the pass that stopped.
 */
...
export function readFetch(
	...
	return handlerFetch(tenantReadPath(pathname), init, envOverride);
}
...
// A per-call binding override lets a test observe or fault what the request
// reaches; the tenant read app runs in process here, so it sees the same
// bindings.
...
the suffix under `/cache/<name>` for a named one.
*/
