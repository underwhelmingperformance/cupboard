...
// Every control trust rule, mapped to the shape the OIDC matcher reads. A
// control rule grants the interactive control admin (a wildcard over the control
// domain); the pinned `sub` lives in the claims map and is matched exactly like
// any other claim, so the tenant matcher serves the control plane unchanged.
//
...
// the system, handed out on issuer membership alone. A rule that does not pin a
// non-empty `sub` is rejected (fail closed), so no unpinned rule can ever issue
// an admin token, whatever wrote it.
