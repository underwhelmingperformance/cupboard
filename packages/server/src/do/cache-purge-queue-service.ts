...
// A stored row is parsed for corruption only, never for length: see the note on
// `purgeEntriesSchema` in `signing-keys-service.ts`. A row is never written
// empty, so an empty array is corrupt.
