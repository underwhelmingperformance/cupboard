		...
		const migrated = await runInDurableObject(
			...
			async (_instance, state) => {
				...
				const overrides = state.storage.sql
					.exec(
						'SELECT cache_id, root_prefix, ttl_seconds FROM cache_root_ttl_override ORDER BY cache_id, root_prefix'
					)
					.toArray();
					...
					cacheSummarySchema.parse({
						...
						defaultRootTtl:
							row.default_root_ttl_seconds === null
								...
								: {
										kind: 'duration',
										ttlSeconds: row.default_root_ttl_seconds
									},
						grace:
			...
			summaries: [
				{
					...
					rootTtlOverrides: [
						{ rootPrefix: 'ci/', ttlSeconds: 100 },
						{ rootPrefix: 'pr/', ttlSeconds: 200 }
					],
					graceManaged: false
				},
				{
