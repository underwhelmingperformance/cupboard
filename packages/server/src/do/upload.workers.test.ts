...
describe('upload flow', () => {
	...
	// The per-pull-request workflow pushes to `pr-<number>` without creating
	// the cache first. The new cache takes the access of the tenant's default
	// cache, so a tenant whose reads are private gets a private `pr-7`.
	it.each([
		...
		async ({ defaultAccess }) => {
			...
			const listed = await authorisedFetch('/caches', init.token);
			const caches = cacheListResponseSchema.parse(await listed.json());

			expect(
				caches.caches.filter((entry) => entry.scope.kind === 'named')
			).toStrictEqual([
				{
					scope: cache,
	...
	// A failed object delete leaves the published object and its reference edge
	// intact, although the local row is already removed. The queued deletion
	// lets collection finish the cleanup and revoke the NAR's read authority.
