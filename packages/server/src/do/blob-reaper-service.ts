...
export class BlobReaperService {
	constructor(
		...
		private readonly subrequestAllowance: number
	) {}
		...
		switch (phase) {
			case 'delete-existing': {
				const result = await drainObjectDeletions(
					...
					this.blobs,
					kind,
					Math.min(limit, objectDeletionBatchSize(this.subrequestAllowance))
				);
			...
			case 'recover': {
				const result = await recoverAbandonedIncarnations(
					...
					kind,
					now,
					Math.min(limit, objectRecoveryBatchSize(this.subrequestAllowance)),
					this.subrequestAllowance,
					logger
				);
