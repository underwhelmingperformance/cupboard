...
describe('transition gate', () => {
	...
	it('reads each transition from its own row', async () => {
	...
	it('returns a result from a fresh reading after a refresh', async () => {
	...
	it.each([
		{
			name: 'ignores a transition id that this build does not define',
			id: 'later-transition',
			state: 'complete',
			reached: { expanded: false, complete: false }
		},
		{
			name: 'counts a state that this build does not define as complete for a known transition',
			id: 'cache-identity',
			state: 'later-state',
			reached: { expanded: true, complete: true }
		}
	])('$name', async ({ id, state, reached: expected }) => {
		await recordRow(id, state);

		expect(await reached(gateOver())).toStrictEqual(expected);
	});

	it('throws a retryable error after the read fails twice', async () => {
