...
export class RetentionService {
	...
	addPolicy(body: ParsedRetentionPolicyAddBody): RetentionPolicySummary {
		...
		// A cache-scoped policy can name a cache that does not exist yet. Its
		// `cache_id` is null until `CacheRepository.ensure` creates the identity
		// and links the row.
		const identity =
			body.scope === 'cache'
				? {
	...
	markCacheGraceManaged(
		...
		writer
			...
			.run();
		new CacheRepository(writer).markGraceManaged(cache);
	}
