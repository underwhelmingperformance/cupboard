...
export async function ensureRetentionRuleSet(
	...
	return database.transaction((tx) => {
		...
		// The rules travel as one JSON parameter, so a rule set of any size binds
		// the same parameters. The check constraint admits a TTL of at least one
		// second, so zero stands for the permanent rules that store none.
