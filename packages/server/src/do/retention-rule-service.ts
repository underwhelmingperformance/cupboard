...
import { CacheRetentionRuleLimitExceededError } from '../errors.ts';
...
export async function ensureRetentionRuleSet(
	...
	return database.transaction((tx) => {
		...
		// Each chunk uses one JSON parameter. Permanent rules carry zero in the
		// list; `nullif` stores NULL, as the check constraint requires for them.
		const listed = canonical.map((rule) => ({
			rootPrefix: rule.rootPrefix,
			kind: rule.retention.kind,
			ttlSeconds:
				rule.retention.kind === 'duration' ? rule.retention.seconds : 0
		}));
		...
		for (const rows of jsonRowLists(listed)) {
			tx.insert(schema.rootRetentionRules)
				.select(
					rows.insertSource([
						sql`${ruleSetId}`,
						rows.column('rootPrefix'),
						rows.column('kind'),
						sql`nullif(${rows.column('ttlSeconds')}, 0)`
					])
				)
				.run();
		}
...
export class RetentionRuleService {
	...
	listForRuleSet(
		ruleSetId: (typeof schema.cacheIdentities.$inferSelect)['rootRetentionRuleSetId']
	): readonly RootRetentionOverride[] {
	...
	listForCache(cache: ResolvedCache): readonly RootRetentionOverride[] {
		const row = this.context.db
			.select({ ruleSetId: schema.cacheIdentities.rootRetentionRuleSetId })
	...
	async setRule(
		...
		const updated = [...current, { rootPrefix, retention }];

		if (updated.length > maxRootRetentionOverrides) {
			throw new CacheRetentionRuleLimitExceededError(maxRootRetentionOverrides);
		}

		await this.bind(cache, updated);
	}
