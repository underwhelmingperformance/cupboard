 ...
 * alarm does not cancel that delivery. A delivery would advance the walk under
 * the alarm's own budget instead of the one these tests set. Each driven pass
 * therefore removes the continuation, so a stray delivery finds no work and
...
// A pass under a one-row budget advances the walk by one step (seeding roots,
...
// hold at most three paths, so sixteen passes is ample. The bound is reached
// only when the walk has wedged; the assertions after the loop then fail, and
// their output shows the state it reached.
...
describe('garbage collection cap', () => {
	...
	it('spends one budget across the refresh-family and collection phases', async () => {
		...
		await setRoot(token, { name: 'channel', targets: [kept.storePath] });
		await seedExpiredRefreshFamily(phaseStepSize + 1);
		...
		expect({ firstStep, secondStep }).toStrictEqual({
			firstStep: {
				scan: {
					...
					referenceCursor: phaseStepSize - 1
				},
				frontier: phaseStepSize
			},
...
describe('garbage collection identity columns', () => {
	...
	it('records the cache identity on collection state and the verification cursor', async () => {
		...
		await pushPath(token, kept, namedCache('builds'));
		await pushPath(token, collectable, namedCache('builds'));
	...
	// A teardown keeps the cache's identity row, so a run row that still refers
	// to the cache resolves and the pass moves on to the next cache.
	it('advances a tenant-wide pass past a cache torn down between passes', async () => {
		await useTestServer('gc-identity-torn-down');
		const { token } = await bootstrap({
			caches: [{ scope: namedCache('builds') }, { scope: namedCache('docs') }]
		});
		const markers = (): Promise<number> =>
		...
		await authorisedFetch('/caches/builds?force=true', token, {
			method: 'DELETE'
		});
		await driveToCompletion(
			() => currentServer().resumeCacheTeardown(),
			async () => (await markers()) === 0,
			3
		);
		// The run row a pass left pointing at `builds` before its teardown. The
		// identities are numbered in creation order: the default cache, `builds`,
		// `docs`.
		await runInDurableObject(currentServer(), (instance) => {
			instance.context.db
		...
		// The run continues on the alarm until every cache has been walked and
		// the run row is removed.
		const result = await runGcResult();
		await drainContinuation();
		const state = await collectionStateIdentities();

		expect({ ok: result.ok, tenantRun: state.tenantRun }).toStrictEqual({
			ok: true,
			tenantRun: []
		});
	});
