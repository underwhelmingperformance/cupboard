...
describe('legacy private object move', () => {
	it('continues past a full page owned by a public cache named private', async () => {
		...
		const publicKeys = Array.from(
			{ length: 1001 },
			(_, index) =>
				`t/${tenant}/narinfo/private/${String(index).padStart(32, '0')}`
		);
		...
		try {
			...
			expect({
				outcomes: [first.kind, second.kind],
				source: await objectAt(source),
				destination: await objectAt(destination),
				publicObject: await objectAt(publicKeys[0] ?? '')
			}).toStrictEqual({
				outcomes: ['incomplete', 'recorded'],
				source: undefined,
				destination: { body: legacyBody, generation: '3' },
				publicObject: { body: currentBody, generation: '3' }
			});
		} finally {
			await env.BLOBS.delete(publicKeys.slice(0, 1000));
			await env.BLOBS.delete(publicKeys.slice(1000));
			await clear(source, destination);
		}
	});
