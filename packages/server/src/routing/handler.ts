...
function buildApp(): Hono<WorkerHonoEnv> {
	...
	app.use('/t/:tenant/*', async (context, next) => {
		...
		// A public read carries the addressed cache's lifecycle version in its
		// Workers Cache key, and a read inside the private namespace needs that
		// cache's read verifier. Pass the cache parsed from the raw path so
		// admission reads its rows alongside the tenant row, in one D1 batch,
...
// bare path, and the selector's cache under `/cache/<selector>`. A selector
// that does not parse yields `undefined`, and the hints then omit the
// cache-specific reference facts; the Durable Object parses the selector
// itself and refuses the request.
