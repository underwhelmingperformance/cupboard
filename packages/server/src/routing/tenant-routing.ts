 ...
 * `/private-cache/<name>` prefix. Every other path resolves to the default
 * cache: the bare tenant prefix addresses it, and a path that addresses no
 * cache (a reuse view, the write surface) or whose name is malformed gets a
 * version nothing uses, because a later routing stage refuses or ignores it.
 *
 * Admission calls this on the raw path before Hono matches a route, so the
