...
export const blobReference = sqliteTable(
	'blob_ref',
	{
		...
		// The identity of `cache`; the trigger of migration 0023 fills both when
...
//
// `cache_kind` and `cache_name` mirror `cache`. `access` is `private` for a
// `private/` cache and the tenant's read mode otherwise. The triggers of
// migrations 0023 and 0024 fill all three for a row inserted without them and
// keep `access` in step with `tenant.read_mode`, until a later migration drops
// them.
export const cacheLifecycle = sqliteTable(
	'cache_lifecycle',
	{
		...
		// The version of how the cache reads. A deletion advances it; a
		// registration leaves it as it is. The Workers Cache key for a public
		// read carries it.
