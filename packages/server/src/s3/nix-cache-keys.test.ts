...
describe('internalKeyFor', () => {
	it('namespaces a narinfo by tenant and cache and shares the NAR', () => {
		expect(internalKeyFor({ kind: 'narinfo', storePathHash }, tenant, '')).toBe(
			`t/acme/narinfo/${storePathHash}`
		);
		expect(
			internalKeyFor({ kind: 'narinfo', storePathHash }, tenant, builds)
		).toBe(`t/acme/narinfo/builds/${storePathHash}`);
		expect(internalKeyFor({ kind: 'nar', hash: narHash }, tenant, '')).toBe(
			`nar/${narHash}.nar.zst`
		);
	});

	it('refuses to address nix-cache-info', () => {
		expect(() => internalKeyFor({ kind: 'cache-info' }, tenant, '')).toThrow();
	});
});
