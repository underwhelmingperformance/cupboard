...
function fakeBackend(
	...
	return {
		authorizeRead: () => Promise.resolve(),
		authorizeWrite: () => Promise.resolve(),
		...
		resolveServableNar: (hash) => Promise.resolve(hash),
...
describe('createNixCacheObjectStore reads', () => {
	...
	it('resolves a named cache from the key prefix', async () => {
		...
		const authorizeRead: Mock = vi.fn(() => Promise.resolve());
		const store = createNixCacheObjectStore(
			tenant,
			fakeBlobStore(objects),
			fakeBackend({ authorizeRead })
		);
		...
		expect(authorizeRead).toHaveBeenCalledWith('builds', undefined);
	});

	it('answers NoSuchKey for a missing object and a non-cache key', async () => {
		const store = createNixCacheObjectStore(
			tenant,
			fakeBlobStore(new Map()),
			fakeBackend()
	...
	it('answers NoSuchKey for a NAR the tenant does not reference', async () => {
		const objects = new Map([[`nar/${narHash}.nar.zst`, utf8('narbytes')]]);
		const store = createNixCacheObjectStore(
			tenant,
			fakeBlobStore(objects),
...
describe('createNixCacheObjectStore writes', () => {
	it('stages a NAR put by its file hash', async () => {
		...
		expect(stageNar).toHaveBeenCalledWith(fileHash, expect.anything(), anyMeta);
	});
	...
	it('propagates an authorisation denial', async () => {
		const store = createNixCacheObjectStore(
			...
			fakeBlobStore(new Map()),
			fakeBackend({
				authorizeWrite: () => Promise.reject(new RequestNotSignedError())
			})
		);
...
describe('createNixCacheObjectStore multipart', () => {
	...
	it('delegates the multipart lifecycle for a NAR key', async () => {
		...
		await store.uploadPart(narContext, 'up', 1, streamOf(emptyBytes));
	...
	it.each([
		...
		const invoke = {
			...
			uploadPart: () =>
				store.uploadPart(narContext, 'up', 1, streamOf(emptyBytes)),
			completeMultipartUpload: () =>
	...
	it('refuses multipart on a narinfo key', async () => {
		...
		await expect(
			store.uploadPart(narinfoContext, 'up', 1, streamOf(emptyBytes))
		).rejects.toThrow(NonCacheWriteError);
	});
});
...
describe('createNixCacheObjectStore list', () => {
	it('scopes returned keys under the resolved cache', async () => {
		...
		const store = createNixCacheObjectStore(
			tenant,
			fakeBlobStore(new Map()),
			fakeBackend({ list: () => Promise.resolve(listing) })
		);
