...
describe.runIf(platform === 'darwin' || platform === 'linux')(
	'real child processes',
	() => {
		it('signals the process it started', async () => {
			const child = spawnChildProcess(['sh', '-c', 'exec sleep 30'], {
				PATH: '/usr/bin:/bin'
			});
		...
		it('reports a child killed by a signal as a signal rather than a status', async () => {
		...
		it('gives the child the composed environment and nothing else', async () => {
			...
			vi.stubEnv('CUPBOARD_UNSET', 'ambient-value');
		...
		it.each([
			...
			{
				name: 'superviseAttemptedBuild',
				run: async (directory: string) => {
		...
		])('$name uses its default child runner', async ({ run }) => {
			const directory = await runtimeDirectory();
			expect(await run(directory)).toStrictEqual({
				status: 7,
				signal: undefined
			});
			await expect(stat(directory)).rejects.toMatchObject({ code: 'ENOENT' });
