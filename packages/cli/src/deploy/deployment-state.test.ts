...
import { localStep, schemaTransitions } from '@cupboard/protocol/deployment';
...
describe('transition records', () => {
	...
	it('creates the table with the statement that migration 0031 runs', () => {
		const migration = readFileSync(
			path.resolve(
				import.meta.dirname,
				'../../../server/drizzle-d1/0031_deployment_transitions.sql'
			),
			'utf8'
		);
		...
		expect(statement).toBe(ensureTransitionTableStatement);
	});
