...
describe('transition walk', () => {
	...
	// column. Its deploy stops at one of those migrations, and the operator
	// rolls back to this build. The deploy records `contracted_at` before the
	// first contract migration, so both points leave a row that this build
	// refuses.
	it.each([
		{
			name: 'first',
			stopsAt: 'ALTER TABLE tenant DROP COLUMN later',
			applied: ['0005_later.sql']
		},
		{
			name: 'second',
			stopsAt: 'CREATE TABLE later_done',
			applied: ['0005_later.sql', '0006_later_contract.sql']
		}
	])(
		'refuses a later transition whose deploy stopped at its $name contract migration',
		async ({ stopsAt, applied: laterApplied }) => {
			const world = fixture();
			...
			world.interruptAt((statement) => statement.startsWith(stopsAt));
	...
	// The upload happens between the two halves of the deploy. A v0.0.34 or
	// v0.0.35 deploy that runs at the same time can record the contracted phase
	// in that window.
	...
	// A later release may drop `deployment_phase` in its contract migrations.
	// After a rollback to this build, the deploy creates the table again.
	it('writes the compatibility phase when a later release dropped its table', async () => {
		const world = fixture();
		await rollBackFromLaterRelease(world, { state: 'complete' });
		world.database.exec('DROP TABLE deployment_phase');
	...
	// Any failure inside this build's own contract migrations leaves the
	// transition expanded with `contracted_at` set. The next run finishes it.
	it('finishes a contract that stopped part-way on the next run', async () => {
		const world = fixture();
		const later = new Date('2026-01-02T00:00:00.000Z');
		await seedApplied(world.api, '0002_expand.sql', world.writes);
		await prepareTransitions(world.walk([cacheIdentity, independent]), false);
		world.interruptAt((statement) =>
			statement.startsWith('ALTER TABLE tenant DROP COLUMN legacy')
		);
		await expect(
			completeTransitions(world.walk([cacheIdentity, independent]))
		).rejects.toBeInstanceOf(InterruptedWriteError);
		const interrupted = recorded(world.database);
		...
		expect({
			...
			rerun: {
				'cache-identity':
					'complete@2026-01-02T00:00:00.000Z;contracted@2026-01-01T00:00:00.000Z',
				'deployment-transitions': 'complete@2026-01-01T00:00:00.000Z'
			},
			applied: migrations.map((migration) => migration.name),
			phase: 'contracted@5'
	...
	describe('errors before any write', () => {
		...
		it.each([
			...
			async ({ transitions, migrations: artifact, dependent: blocked }) => {
				...
				expect({ caught, writes: world.writes }).toStrictEqual({
					caught: new TransitionIncompleteError(
						'cache-identity',
						blocked,
						'v0.0.34'
					),
		...
		it('stops on a complete transition that lists a migration that the deployment has not applied', async () => {
			...
			expect({ caught, writes: world.writes }).toStrictEqual({
				caught: new TransitionMigrationsMissingError('deployment-transitions', [
		...
		it('stops after the upload when a transition has not expanded', async () => {
			...
			expect({
				...
				caught: new TransitionNotExpandedError('cache-identity'),
				writes: [],
				servingChecks: 0
			});
		});
