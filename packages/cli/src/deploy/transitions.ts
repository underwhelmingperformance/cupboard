 ...
 * reads the states again and repeats the checks on the applied migrations,
 * because the `deployment_phase` row can change during the upload. It then
 * checks that both Workers serve this build, and throws
 * `WorkersNotServingBuildError` while a script is split across versions or
 * still serves an earlier build. Then, in list order, it wakes tenants until
 * every active or suspended tenant has recorded each incomplete transition's
 * contract step, applies the transition's contract migrations and records the
 * transition complete. `LocalStepUnreachedError` reports the tenants below the
 * step.
 */
