...
import { UnknownDeploymentTransitionError } from './deployment-state.ts';
import {
...
describe('reviewed deployment plan', () => {
	...
	it('shows and throws the blocking error when a transition that is not independent follows an incomplete one', async () => {
		...
		expect(thrown).toBeInstanceOf(TransitionIncompleteError);
		expect({
			blocked: observation.kind === 'existing' && observation.blocked,
			thrown: thrown instanceof TransitionIncompleteError && {
				transition: thrown.transition,
				dependent: thrown.dependent,
				completedBy: thrown.completedBy
			},
			...
			rows: [
				...
				},
				{
					label: 'Transition deployment-transitions',
					value:
						"blocked: 'cache-identity' must be complete before this transition can expand; first run cupboard deploy with a release of v0.0.34 or later that does not include 'deployment-transitions'; the deployed release qualifies only if its own deploy stopped before completing 'cache-identity'"
				}
			]
		});
	});
