...
// The statement in migration `0031`, character for character, so a table that
// the deploy creates has the same `sqlite_master` text as one that the
// migration creates. A test compares the two.
...
// The statement in migration `0021`, with `IF NOT EXISTS`. A later release can
// drop the table, and the deploy creates it again before it writes the row
// that v0.0.34 and v0.0.35 read. A test compares the two.
export const ensurePhaseTableStatement = [
	'CREATE TABLE IF NOT EXISTS `deployment_phase` (',
	'\t`id` text PRIMARY KEY NOT NULL,',
	'\t`phase` text NOT NULL,',
...
// `queryRows` drops a row whose value is NULL, and `contracted_at` is NULL until
// the contract migrations start. Coalescing every field keeps each row.
...
// The two names that this build writes; the others appear only in the rank.
export type CompatibilityPhase = Extract<
	(typeof compatibilityPhases)[number],
	'native-reads' | 'contracted'
>;
...
export class UnknownDeploymentTransitionError extends CliError {
	constructor(
		...
		public readonly reason: Exclude<TransitionRefusal, 'contracted'>
	) {
		super(
...
export function refusalError(
	...
	switch (reason) {
		...
		case 'unknown-transition-and-state': {
			return new UnknownDeploymentTransitionError(row.id, row.state, reason);
		}
	}
}
