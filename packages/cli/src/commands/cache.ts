...
export function registerCacheCommands(
	...
	cache
		...
		// No default: `tenant create` makes a tenant's default cache private
		// unless told otherwise, and a public default here would make the two
		// commands disagree about a new cache's access.
		.requiredOption(
			'--access <mode>',
	...
	cache
		.command('inspect')
		.description("Show one cache's access, priority and store-path count.")
		.argument('<url>', tenantUrlArgument, parseWorkerUrl)
		.argument('[name]', 'named cache; omit it for the default cache')
