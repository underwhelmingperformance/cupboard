 ...
 * `/cache/{cacheName}` path segment. No named-cache path addresses the
 * default cache. Where the default cache still needs a selector (token
 * grants, reuse-view selectors, the commit socket path) `selectorForCache`
 * returns `_default` for it.
 */
