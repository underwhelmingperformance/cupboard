...
function applyHistoricalMigrations(
	...
	// The generated bundle names each migration `m0000`, `m0001` and so on, so
	// read it through a keyed record rather than by a fixed property.
	const sources: Readonly<Record<string, string>> = migrations.migrations;

	for (const entry of entries.slice(applied.length)) {
		const source = sources[migrationKey(entry.idx)];
