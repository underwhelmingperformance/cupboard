...
migrations add schema. The new build, and every build that can still be deployed
or rolled back to, must be able to run against it. This is wider than the
preceding build: after a skip-level upgrade, such as one from v0.0.33, the
deployed build is older than the preceding release, and a rollback can cross two
releases. The contract migrations remove what the preceding build reads, and run
once both Workers' deployments send all traffic to the new build. The
transitions are listed in order in `schemaTransitions` in
...
- `deployment-transitions`: migration `0031` creates the `deployment_transition`
  table. It has no contract migrations and no contract step, and it is
  _independent_: its expand migrations do not depend on the contract migrations
  of the transitions before it, and they do not change existing rows. Once every
...
stops part-way still leaves it set, and it stays empty for a transition without
any. A transition with no row is _pending_. The deploy creates
...
1. Read the recorded states, before applying a migration or uploading a Worker,
   ...
   - a row with a transition id or state that this build does not define (a row
     of a transition that this build does not define, in state `expanded` or
     `complete`, whose contract migrations have not started is the exception:
     see "Rolling back");
   ...
   - a transition recorded as complete although one of its migrations is missing
     from `d1_migrations`;
   - a transition that is not independent and follows a transition that is not
     complete.

   A transition that is not independent can expand only once every earlier
   transition is complete. Otherwise its expand migrations could run only after
   the upload, and the new Workers would run without them until then. No
   transition in this release is blocked on any deployment:
   `deployment-transitions` is independent, so a deployment on v0.0.33 upgrades
   directly. A later release that adds a dependent transition can be blocked.
   Its error gives the releases that complete the earlier transition and do not
   include the later one: for `cache-identity`, v0.0.34 or a later release
   without that transition, such as this one. The deployed release qualifies
   only if it is one of them and its own deploy stopped before completing the
   earlier transition. When that deploy has finished, deploy the later release.
   A fresh deployment is exempt, because every transition completes on it before
   the upload.
...
`contracted`, the next run writes `contracted`. The deploy also replaces a phase
name that no release wrote. This build's Workers do not read the row. The deploy
reads it to reconcile the recorded transitions.
...
treats such a row by its `contracted_at`, which the deploy sets before the first
contract migration runs:

- `contracted_at` empty, with state `expanded` or `complete`: the later release
...
- `contracted_at` set, in any state: the transition's contract migrations have
  started and may have removed schema that this build needs, so the deploy stops
  with an error before it changes anything. Stay on the deployed release, and
  use its `cupboard deployment status` and `cupboard deployment resume` for any
  remaining tenant work. To roll back to this build anyway, first confirm from
  that release's migrations that its contract migrations remove nothing that
  this build reads. Then clear `contracted_at` with `wrangler d1 execute`:
  `UPDATE deployment_transition SET contracted_at = NULL WHERE id = '<id>';`,
  and deploy this build again.
- `contracted_at` empty, with a state that this build does not define: the
  deploy stops with an error. Deploy a build that defines the transition and the
  state.
...
list all such rows under `unrecognised` in the `deployment.transitions` control
procedure, and `cupboard deployment status` prints each one with what this
build's deploy does with it. The tenant objects and the control-plane trust gate
ignore a row with an unknown transition id, and count an unknown state of a
known transition as complete, because a later release may only add states after
`complete`.
